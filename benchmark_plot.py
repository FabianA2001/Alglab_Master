...
def gather_sol_times(data_dir):
    ...
    dict_constraint = {
        ...
        # 47: "1S0_wv0_o0_1Scp0.025_wvcp0.012_ocn0_alt",
        # 48: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer0.5_lns",
        # 49: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer0.5_onlylns",
        # 50: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer3_lns",
        # 51: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer3_onlylns",
        # 52: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer30_lns",
        # 53: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer30_onlylns",
        # 54: "1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer45_it2_if2_dif_onlylns",
        # 55:"1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer45_it2_if2_dif_lns",
        # 56:"1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer45_it2_if2_dif_lns_alt",
        # 57:"1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer60_it3_if2_onlylns",
        # 58:"1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer60_it3_if2_lns",
        # 59:"1S10_wv10_o30_1Scp0.025_wvcp0.012_ocn0_timer60_it3_if2_lns_alt",
    }
    ...
    for key in dict_constraint.values():  # Modify keys as per your dictionary
        for try_num in range(0, 5):  # Assuming tries are 1 and 2
            ...
            for file in file_pattern:
                solution = Solution.from_json_file(
                    file.stem
                )  # Load using the base filename
                instance_name = file.stem.split("_")[
                    0
                ]  # Get instance name from filename
        ...
        with open("no_opt_solution_found.txt", "r") as file:
            for line in file:
                ...
                if line:  # Ensure the line is not empty
                    instance_name = line.split("_")[
                        0
                    ]  # Extract instance name from the filename
...
def plot_sol_times_lines(df, solution_type, value_type):
    ...
    for measure in measures:
        ...
        if measure == "min":
            plot_data = (
                df.groupby(["instance", "key"])[value_type]
                .min()
        ...
        for key in plot_data["key"].unique():
            ...
            plt.plot(
                ...
                linestyle="",
                markersize=10,
                label=f"{measure.capitalize()} {key}",
            )

        plt.title(
            f"Solve {measure.capitalize()} for {solution_type.capitalize()} {value_type.capitalize()} Solutions"
        )
...
def plot_sol_times_barchart(df, solution_type, value_type):
    ...
    for measure in measures:
        ...
        if measure == "min":
            ...
            print(
                df.groupby(["instance"])[value_type].median().reset_index(name="value")
            )
        elif measure == "count":
            plot_data = (
                df.groupby(["instance", "key"])[value_type]
...
def aggregate_scores(df, value_type, solution_type):
    ...
    for instance in instances:
        ...
        for metric in ["min", "max", "mean", "median"]:
            ...
            sorted_metric["score"] = (
                sorted_metric[metric].rank(method="dense").astype(int)
            )
            ...
            for row in sorted_metric.itertuples():
                ...
                if method not in method_scores:
                    method_scores[method] = {
                        metric: 0 for metric in ["min", "max", "mean", "median"]
                    }
                method_scores[method][metric] += (
                    score  # Summing scores across instances
                )
    return method_scores
        ...
        for type in ["objective_value", "solve_time"]:
            if len(df_sol_times) > 0:
                ...
                if len(filtered_df) > 0:
                    ...
                    plot_sol_times_lines(
                        filtered_df, solution_type=solution_type, value_type=type
                    )
                    plot_sol_times_barchart(
                        filtered_df, solution_type=solution_type, value_type=type
                    )
