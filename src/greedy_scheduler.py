...
class SequentialGreedyScheduler2:
    def __init__(self, instance: instace.Instance):
        self.instance = instance
        ...
        self.shifts_per_type = {emp: defaultdict(int) for emp in instance.employees}
        self.last_shift_type = {emp: 0 for emp in instance.employees}
        # Track weekends assigned (set of weekend indices per employee)
        self.weekends_assigned = {emp: set() for emp in instance.employees}
    ...
    def greedy_score(self, emp, day, t):
        ...
        # 2b. This employee is banned for this specific shift
        if emp in shift.ban_employee_day_shift:
            return math.inf
        ...
        # 4. Max shifts of this type
        max_s = emp_obj.max_numbers_of_shifts[t_obj.uid]
        if self.shifts_per_type[emp][t] >= max_s:
            return math.inf
        ...
        # 5. Max consecutive shifts: ensure assigning today does not create a run > allowed
        # Count consecutive working days directly before 'day'
        prev_cons = 0
        for i in range(1, emp_obj.max_number_consecutive_shifts + 1):
            d = day - i
            if d < 0:
                break
            worked = any(emp in self.x.get((d, ty), []) for ty in self.shift_types)
            if worked:
                prev_cons += 1
            else:
                break
        if prev_cons + 1 > emp_obj.max_number_consecutive_shifts:
            return math.inf

        # Minimum consecutive shifts: if we're starting a new working block, ensure enough future
        # days exist to satisfy the minimum length (simple forward-feasibility check)
        if emp_obj.min_number_consecutive_shifts > 1:
            # are we starting a new block? if previous day not worked
            prev_day = day - 1
            starting_new = True
            if prev_day >= 0:
                starting_new = not any(
                    emp in self.x.get((prev_day, ty), []) for ty in self.shift_types
                )
            if starting_new:
                needed = emp_obj.min_number_consecutive_shifts
                # check we have enough days remaining
                if day + (needed - 1) > (self.num_days - 1):
                    return math.inf
                # check simple feasibility on following days (not blocked)
                feasible = True
                for j in range(1, needed):
                    future_day = day + j
                    if future_day in emp_obj.blocked_shifts:
                        feasible = False
                        break
                if not feasible:
                    return math.inf
        ...
        if (
            ...
            score += self.instance.shifts[day][t].penalty_not_assigned_day_employee[emp]
        # A. reach minimum minutes: encourage assigning employees that are below their minimum
        if self.total_minutes[emp] < emp_obj.min_minutes_assigned:
            diff = emp_obj.min_minutes_assigned - self.total_minutes[emp]
            # decrease score (more attractive) proportional to missing minutes
            score -= diff / 100.0
        ...
        if current_cover < desired:
            # prefer employees for under-covered shifts: reduce score by missing amount * weight
            score -= shift.weight_below_preferred * (desired - current_cover)
        # else:
        #     score += shift.weight_above_preferred * (current_cover - desired)
    ...
    def schedule(self):
        ...
        Verbesserter Greedy: Füllt jede Schicht iterativ mit mehreren Mitarbeitern bis zum Soll.
        """
        # Prefill mandatory assignments from instance (assign_employee_day_shift)
        for day in range(self.num_days):
            for t in self.shift_types:
                shift = self.instance.get_shift(day, t)
                for emp in getattr(shift, "assign_employee_day_shift", set()):
                    # skip if already assigned that day to some other shift
                    already = any(emp in self.x[(day, ty)] for ty in self.shift_types)
                    if already:
                        continue
                    # commit required assignment
                    self.x[(day, t)].append(emp)
                    self.shifts_per_type[emp][t] += 1
                    self.total_minutes[emp] += self.shift_types[t].length
                    self.last_shift_type[emp] = t
                    self.cover[(day, t)] += 1
                    if getattr(shift, "is_weekend", False):
                        self.weekends_assigned[emp].add(day // 7)
        ...
        for day in range(self.num_days):
            for t in self.shift_types:
                ...
                while len(assigned) < preferred:
                    ...
                    self.cover[(day, t)] += 1
                    if getattr(shift, "is_weekend", False):
                        self.weekends_assigned[best_emp].add(day // 7)
                    assigned.add(best_emp)
        return dict(self.x)
