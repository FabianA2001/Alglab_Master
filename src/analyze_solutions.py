...
def analyze_solution_quality(solution: Solution) -> dict:
    ...
    wish_satisfaction_rate = (satisfied_wishes / total_wishes * 100) if total_wishes > 0 else 0
    ...
    for day, day_shift_dict in instance.shifts.items():
        for type_uid, shift in day_shift_dict.items():
            ...
            assigned = sum(
                1 for (d, t, e), v in vars_map.items()
            ...
            shift_utilization.append({
                ...
                'below_preferred': solution.below_prefferd_vars.get((day, type_uid), 0),
                'above_preferred': solution.above_prefferd_vars.get((day, type_uid), 0)
            })
    
    overall_utilization = (total_assigned / total_preferred * 100) if total_preferred > 0 else 0
    ...
    return {
        ...
        'wishes': {
            ...
            'satisfaction_rate': satisfied_wishes / total_wishes if total_wishes > 0 else 0,
        },
        ...
        'shifts': {
            ...
            'overall_utilization': total_assigned / total_preferred if total_preferred > 0 else 0,
...
def print_analysis_summary(analysis_results: dict) -> None:
    ...
    for solution_name, analysis in sorted(analysis_results.items()):
        ...
        print(f"  Größe: {analysis['num_days']} Tage, {analysis['num_employees']} Mitarbeiter, {analysis['num_shift_types']} Schichttypen")
        ...
        for item in details[:5]:
            if item['utilization_percent'] < 100:
                ...
                print(f"    {shift_name}: {item['assigned']}/{item['preferred']} ({item['utilization_percent']:.1f}%)")
...
def save_analysis_to_json(analysis_results: dict, output_path: Optional[Path] = None) -> None:
    """
    ...
    if output_path is None:
        output_path = Path(__file__).resolve().parent.parent / "data" / "analyses_summary.json"
