...
class Minimum_consecutive_shifts(ShiftAssignmentModule):
    def build(
        ...
        for employee_uid in instance.employees:
            for day_s in range(instance.employees[employee_uid].min_number_consecutive_shifts-1 -1):
                for day_d in range(instance.number_of_days - (day_s + 1) -1):
                    ...
                    for type_uid in instance.shift_types:
                        ...
                        assigned_shifts_interval_end.append(vars.vars[(day_d + day_s + 1 + 1, type_uid, employee_uid)])
                    # Even though our indecies start with 0, day_s should still have the start value of 1
                    vars.model.add(sum(assigned_shifts) + day_s + 1 - ( sum(assigned_shifts_inner_interval)) + sum(assigned_shifts_interval_end) > 0)
        return 0
