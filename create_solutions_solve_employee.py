        ...
        for json_file in json_files_best_till_time:
            for x in range(0, 3):
                for percentual_improvement_shift, time_between_checks_in_seconds_shift, percentual_improvement_work_var, time_between_checks_in_seconds_work_var, numerical_improvement_opt in [(0.025,8,0.012,8,0)]:
                    ...
                    solution = solver_employee.solve_instance_one_shift(one_shift_max_time=one_shift_time*60, fixed_work_var_opt_max_time=work_var_time*60, general_optimization_max_time=0*60, one_shift_callback=callback_one_shift,fixed_work_var_opt_callback=callback_opt_work_var)
                    ...
                    for not_better_stop_timer in [30, 3]:
                        lns_start_time = time.time()
                        solution_lns = lns.LNS(sol_or_instance=solution.model_copy(deep=True), timeout_seconds=timeout_seconds, log_level=20).solve(not_better_break_after=not_better_stop_timer*60)
                        ...
                        solution_opt = Solver(instance.model_copy(deep = True), Shift_vars(instance.model_copy(deep = True))).warm_start_generalized(hint_solution=solution_lns.model_copy(deep=True), max_time_in_seconds=opt_time*60-(time.time()-start_time))

                        solution_opt.solve_time = (time.time() - lns_start_time) + first_opt_time
                        ...
                        if solution_opt.solve_status in [cp_model.FEASIBLE] or solution_opt.solve_status in [cp_model.OPTIMAL]:
