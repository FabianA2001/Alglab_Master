        ...
        for json_file in json_files_best_till_time:
            for x in range(0, 3):
                for percentual_improvement_shift, time_between_checks_in_seconds_shift, percentual_improvement_work_var, time_between_checks_in_seconds_work_var, numerical_improvement_opt in [(0.025,8,0.012,8,0)]:
                    ...
                    if opt_time != 0:
                        if solution.solve_status in [cp_model.OPTIMAL]:
                            filename = f"{solution.instance.name}_1S{one_shift_time}_wv{work_var_time}_o{opt_time}_1Scp{percentual_improvement_shift}_wvcp{percentual_improvement_work_var}_ocn{numerical_improvement_opt}_alternative_opt_{x}"
                            solution.to_json_file(filename)
                        elif solution.solve_status in [cp_model.FEASIBLE]:
                            filename = f"{solution.instance.name}_1S{one_shift_time}_wv{work_var_time}_o{opt_time}_1Scp{percentual_improvement_shift}_wvcp{percentual_improvement_work_var}_ocn{numerical_improvement_opt}_alternative_time_out_30_{x}"
                            solution.to_json_file(filename)
                        else:
                            error_filename = f"{solution.instance.name}_1S{one_shift_time}_wv{work_var_time}_o{opt_time}_{x}"
                            with open('error_log.txt', 'a') as error_file:
                        ...
                        filename = f"{solution.instance.name}_1S{one_shift_time}_wv{work_var_time}_o{opt_time}_1Scp{percentual_improvement_shift}_wvcp{percentual_improvement_work_var}_ocn{numerical_improvement_opt}_alternative_immediate_first_{x}"
                        solution.to_json_file(filename)
