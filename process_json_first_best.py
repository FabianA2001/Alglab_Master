...
def process_file(json_file, x, constraint_number, key):
    ...
    filename = f"{instance.name}_{key}_first_good_{x}"  # Include x in the filename
    print("\n" + filename)

    solution = solver.solve_with_early_stop(log_search_progress=False, max_time_in_seconds=30*60, constraint_set = constraint_number).model_copy(deep=True)
